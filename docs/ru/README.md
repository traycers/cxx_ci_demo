...
5. `docker compose run --rm bootstrap` — создаёт 6 репозиториев в GitLab (`ci-infra` и пять `project_*`), пушит в них посевное содержимое из `repos/<repo>/<branch>/` и направляет versioned settings TeamCity на `ci-infra`. Работает как одноразовый контейнер, подключённый напрямую к сети `cxxci` (см. ADR 0008), а не как хостовой скрипт — поэтому ничего здесь не зависит от версий `curl`/`git`/`docker` на хосте. Безопасно перезапускать.
...
- **"Test connection"/сборка VCS root падает с `HTTP Basic: Access denied` или
  ...
  может выглядеть похоже. Если это бьёт конкретно по одному из VCS root'ов `project_*`,
  ...
  того, как появился `CxxCiDemo_Main_ProjectA`, то есть только после того, как versioned
