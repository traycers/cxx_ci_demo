...
1. A clear feature-branch model: switch the repos to the new branch and the track "slice" is ready.
2. Environment setup via the system package manager.
3. Future tracks can change their own dependency tree.
4. Fast creation of new tracks, by copying the previous one.
...
4. Sustaining this process will require writing and maintaining scripts (fetching and building the repos) across every branch, including old tracks. There's a real chance these scripts degenerate into writing a package manager of their own. Solved by keeping them in one shared repo.
...
7. Tracks are independent of each other: even a repo that hasn't changed from one track to the next still gets its own build run in every track. Builds can't be reused across tracks, which means storage has to be sized for every track's build artifacts, not just the ones that changed.
