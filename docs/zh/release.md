[🇬🇧 English](../en/release.md) · [🇷🇺 Русский](../ru/release.md) · 🇨🇳 中文

_翻译自 `docs/en/release.md`。原文变更时请同步更新本文件——参见 [ADR 0006](adr/0006-trilingual-docs-mirror-tree.md)。_

# Release

**Release** 是发布一个版本所需要的若干个项目的集合。

把 release 组织成独立的目录，是为了让创建新 release(复制并调整一个已有的)、删除已经停止维护的 release、以及让各个 release 互不影响都变得简单。

## 内容

Release 有自己的名字，这个名字在 GitLab 和 TeamCity 中保持一致。

Release 的默认分支名为 `<release_name>`。git 中其余的分支都遵循 `<release_name>-*` 这个命名模式，TeamCity 的 VCS root 用的是同一个模式。

向分支提交代码会触发 TeamCity 中的构建；根据过滤规则，只有这个 release 自己的构建链会被触发。要做修改，开发者需要在自己需要改动的那些仓库里，创建一个同名的分支。如果某个仓库里没有这个名字的分支，TeamCity 就会改用默认分支的构建产物。

`result` 构建是这个 release 的总体结果——触发它的，是基于 VCS 提交的触发器。

> 重要！
> release 在 GitLab、TeamCity 和 git 分支里用的是同一个名字。
...
```mermaid
flowchart TD
    A["Release 创建<br/>(在 ci-infra 中定义 config_name)"] --> B["在每个需要的仓库中<br/>创建默认分支:refs/heads/&lt;release_name&gt;"]
    B --> C{"需要改动吗?"}
    C -- "不需要——直接提交到默认分支" --> D["Push 到 refs/heads/&lt;release_name&gt;"]
    C -- "需要——feature/hotfix" --> E["只在需要的仓库中<br/>创建 refs/heads/&lt;release_name&gt;-*"]
    E --> F["Push 到 refs/heads/&lt;release_name&gt;-*"]
    D --> G["TeamCity 中触发 VCS 触发器<br/>只过滤出这一个 release"]
    F --> G
