...
- **`docker compose up` 在挂载 `BUILDAGENT_DATA_DIR/*`时失败**(permission denied)：docker 守护进程需要能够创建/拥有这个目录——`.env.example` 默认将其设为 `./agents_dir`(已加入 `.gitignore`/`.dockerignore`），正是为了让它始终位于你已经拥有的 checkout 目录之内；但如果你把它指向了别处(例如改回 `/opt/buildagent`)，守护进程就又需要在那里创建/拥有目录的权限——对普通的 rootful Docker 安装成立，但对 rootless Docker 或没有 root 权限的宿主机账号不成立。请在 `.env` 中将 `BUILDAGENT_DATA_DIR` 设置为你实际拥有权限的目录，然后重新运行。这些路径必须是宿主机 bind mount，而不能是命名卷(named volume)——原因见 `docker-compose.yml` 中 `teamcity-agent` 上的注释。
