...
object Track1_BuildCImage : BuildType({
    id((Track1Id / "BuildCImage").toString())
    name = "Build C++ image"
    ...
    steps {
        script {
            name = "docker build"
            scriptContent = "docker build -t cxxci-build:${Track1TrackName}-%build.number% -f .teamcity/cxx_ci_demo/${Track1TrackName}/Dockerfile .teamcity/cxx_ci_demo/${Track1TrackName}"
        }
        ...
        // cxxci-build:Track1-* images (by build number, not by age — a re-triggered old build
        ...
        script {
            ...
            scriptContent = """
                docker images --format '{{.Tag}}' 'cxxci-build:${Track1TrackName}-*' \
                    | sed 's/^${Track1TrackName}-//' \
                    | sort -n -r \
                    | tail -n +${'$'}(( %keep_images_count% + 1 )) \
                    | while read -r n; do docker rmi -f "cxxci-build:${Track1TrackName}-${'$'}n"; done
                exit 0
            """.trimIndent()
        }
    }
