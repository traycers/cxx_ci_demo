...
//
// Dockerfile lives at cxx_ci_demo/release_3/Dockerfile — i.e. inside this release's own directory, so
// copying release_3/ to start a new release brings its Dockerfile along too (it can then diverge:
...
object Release3_BuildCImage : BuildType({
    id((Release3Id / "BuildCImage").toString())
    name = "Build C++ image"
    ...
    steps {
        script {
            name = "docker build"
            scriptContent = "docker build -t cxxci-build:${Release3ConfigName}-%build.number% -f .teamcity/cxx_ci_demo/${Release3ConfigName}/Dockerfile .teamcity/cxx_ci_demo/${Release3ConfigName}"
        }
        ...
        script {
            ...
            scriptContent = """
                docker images --format '{{.Tag}}' 'cxxci-build:${Release3ConfigName}-*' \
                    | sed 's/^${Release3ConfigName}-//' \
                    | sort -n -r \
                    | tail -n +${'$'}(( %keep_images_count% + 1 )) \
                    | while read -r n; do docker rmi -f "cxxci-build:${Release3ConfigName}-${'$'}n"; done
                exit 0
            """.trimIndent()
        }
    }
