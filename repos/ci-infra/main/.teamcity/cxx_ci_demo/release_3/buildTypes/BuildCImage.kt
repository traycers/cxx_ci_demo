...
object Release3Track_BuildCImage : BuildType({
    ...
    steps {
        script {
            name = "docker build"
            scriptContent = "docker build -t cxxci-${Release3TrackName}:%build.number% -f .teamcity/cxx_ci_demo/${Release3TrackName}/Dockerfile .teamcity/cxx_ci_demo/${Release3TrackName}"
        }
        ...
        // cxxci-release_3:* images (by build number, not by age — a re-triggered old build
        ...
        script {
            ...
            scriptContent = """
                docker images --format '{{.Tag}}' 'cxxci-${Release3TrackName}:*' \
                    | sort -n -r \
                    | tail -n +${'$'}(( %keep_images_count% + 1 )) \
                    | while read -r n; do docker rmi -f "cxxci-${Release3TrackName}:${'$'}n"; done
                exit 0
            """.trimIndent()
        }
    }
