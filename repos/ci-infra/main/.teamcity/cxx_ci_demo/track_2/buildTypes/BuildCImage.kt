...
//
// Dockerfile lives at cxx_ci_demo/track_2/Dockerfile — i.e. inside this track's own directory, so
// copying track_2/ to start a new track brings its Dockerfile along too (it can then diverge:
...
object Track2_BuildCImage : BuildType({
    id((Track2Id / "BuildCImage").toString())
    name = "Build C++ image"
    ...
    steps {
        script {
            name = "docker build"
            scriptContent = "docker build -t cxxci-build:${Track2TrackName}-%build.number% -f .teamcity/cxx_ci_demo/${Track2TrackName}/Dockerfile .teamcity/cxx_ci_demo/${Track2TrackName}"
        }
        ...
        // cxxci-build:track_2-* images (by build number, not by age — a re-triggered old build
        ...
        script {
            ...
            scriptContent = """
                docker images --format '{{.Tag}}' 'cxxci-build:${Track2TrackName}-*' \
                    | sed 's/^${Track2TrackName}-//' \
                    | sort -n -r \
                    | tail -n +${'$'}(( %keep_images_count% + 1 )) \
                    | while read -r n; do docker rmi -f "cxxci-build:${Track2TrackName}-${'$'}n"; done
                exit 0
            """.trimIndent()
        }
    }
