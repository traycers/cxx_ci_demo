...
// without a parameter (see map.md's decision: dependency() targets a specific BuildType object,
// so a single parameterized template couldn't guarantee a debug build never links a release
// upstream — the whole tree is duplicated per variant instead).
object Main_Release_BaseBuild : Template({
    id((Main_ReleaseId / "BaseBuild").toString())
    name = "base_build"
