...
//
// Dockerfile lives at cxx_ci_demo/release_2/Dockerfile — i.e. inside this release's own directory, so
// copying release_2/ to start a new release brings its Dockerfile along too (it can then diverge:
...
object Release2_BuildCImage : BuildType({
    id((Release2Id / "BuildCImage").toString())
    name = "Build C++ image"
    ...
    steps {
        script {
            name = "docker build"
            scriptContent = "docker build -t cxxci-build:${Release2ConfigName}-%build.number% -f .teamcity/cxx_ci_demo/${Release2ConfigName}/Dockerfile .teamcity/cxx_ci_demo/${Release2ConfigName}"
        }
        ...
        script {
            ...
            scriptContent = """
                docker images --format '{{.Tag}}' 'cxxci-build:${Release2ConfigName}-*' \
                    | sed 's/^${Release2ConfigName}-//' \
                    | sort -n -r \
                    | tail -n +${'$'}(( %keep_images_count% + 1 )) \
                    | while read -r n; do docker rmi -f "cxxci-build:${Release2ConfigName}-${'$'}n"; done
                exit 0
            """.trimIndent()
        }
    }
