...
object Release2Track_BuildCImage : BuildType({
    ...
    steps {
        script {
            name = "docker build"
            scriptContent = "docker build -t cxxci-${Release2TrackName}:%build.number% -f .teamcity/cxx_ci_demo/${Release2TrackName}/Dockerfile .teamcity/cxx_ci_demo/${Release2TrackName}"
        }
        ...
        // cxxci-release_2:* images (by build number, not by age — a re-triggered old build
        ...
        script {
            ...
            scriptContent = """
                docker images --format '{{.Tag}}' 'cxxci-${Release2TrackName}:*' \
                    | sort -n -r \
                    | tail -n +${'$'}(( %keep_images_count% + 1 )) \
                    | while read -r n; do docker rmi -f "cxxci-${Release2TrackName}:${'$'}n"; done
                exit 0
            """.trimIndent()
        }
    }
